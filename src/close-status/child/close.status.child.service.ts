...
export class CloseStatusService extends TypeOrmCrudService<CloseStatusEntity> {
  ...
  async getCloseStatusByCodeNumber(
    ...
    return statuses.map(closeStatus => ({ ...closeStatus }));
  }
  ...
  async createCloseStatusData(payload: CloseStatusDto) {
    try {
      ...
      if (closeStatusEntity?.id) {
        closeStatusEntity.processor = payload.processor;
        closeStatusEntity.isClosed = payload.isClosed;
        this.repo.update(closeStatusEntity.id, {
          ...closeStatusEntity,
          isGuardianOne:
            !payload?.isGuardianOne || payload?.isGuardianOne === 'true'
              ? 'true'
              : 'false',
          isGuardianTwo:
            !payload?.isGuardianTwo || payload?.isGuardianTwo === 'true'
              ? 'true'
              : 'false',
          isChild:
            !payload?.isChild || payload?.isChild === 'true' ? 'true' : 'false',
        });
      } else {
    ...
    try {
      ...
      if (closeStatusDeleted?.affected > 0) {
        ...
        await this.repo
          ...
          .delete()
          .from('background_metadata_entity')
          .where('codeNumber = :codeNumber', { codeNumber })
          .execute();
        await this.repo
