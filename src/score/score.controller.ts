...
export class ScoreController {
  ...
  async getFilterResult(
    ...
    const numOfClients = await this.service.numOfClients(codeNumbers, payload.startDate, payload.endDate);
    const percentage = numOfClients / (await this.backgroundMetadataService.count()) * 100;
    const { ors, score15 } = await this.service.avgOfOrsAndScore15();
